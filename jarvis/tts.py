"""Local TTS via Piper (isolated in jarvis/.venv — never touches system Python).
...
PIPER_BIN = PROJECT_ROOT / "jarvis" / ".venv" / "bin" / "piper"
...
def synthesize(text: str, model_path: str, timeout: int = 60) -> bytes:
    ...
    if not PIPER_BIN.exists():
        raise TTSError(
            ...
            f"python3 -m venv jarvis/.venv && jarvis/.venv/bin/pip install piper-tts"
        )
