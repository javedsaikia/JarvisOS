...
else's name. There is no `hey_orin`, so as long as the wake word is a
...
orin", "computer", your dog's name — with no model to train and nothing
...
- Detection lands at the *end* of the utterance instead of mid-phrase,
  ...
  utterance is already recorded and transcribed, so "hey orin, what's the
...
- Whisper is not a keyword spotter, so it renders the name however it
  hears it: "Hey Orin" comes back as "Hey Oren", and sometimes "Hey,
  Orange." PHRASE_VARIANTS exists for exactly that, the same way the
  loop's existing wake-phrase matching already allows for "jervis".
...
# bare name at the very start ("Orin, check my calendar"), which is how
...
class PhraseWake:
    ...
    def __init__(self, phrases: list[str], stt_model_size: str = "", key: str = "wake",
        ...
        #   accept on its own at the start of an utterance ("Orin, ...").
        ...
        # "Hey Orin": Orange, Arryn, AORING, Arring, Oren, Orrin. The
    ...
    def _name_at(self, tokens: list[str], i: int, loose: bool = True) -> bool:
        ...
        measured: "Orin, check my calendar" came back as "or in check my
    ...
    def _matches(self, transcript: str) -> bool:
        ...
        rather than the exact one:
          - a lead word followed by the name, anywhere in the utterance
            ("okay so, hey orin, what's the weather")
          - the name as the very first word ("orin, check my calendar")
