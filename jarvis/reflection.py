...
{{
  "identity": [{{"text": "Lives in Bangalore", "quote": "i'm in bangalore"}}],
  "speaking_style": [],
  "preferences": [],
  "recurring_topics": []
}}
...
def _format_conversation(turns: list[dict], user_name: str) -> str:
    lines = []
    for turn in turns:
        who = user_name if turn.get("role") == "user" else "Orin"
...
def main():
    """`python3 -m jarvis.reflection [--now|--show|--prune]` — inspect and
