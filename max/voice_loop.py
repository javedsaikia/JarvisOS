"""Always-on voice loop: continuous mic capture -> wake phrase ("Hey Max")
-> record utterance -> local transcription -> normal Max pipeline -> TTS.

Everything here is local: the wake phrase is spotted by VAD + a small
Whisper (max/wake_phrase.py, so the phrase is whatever config says),
...
Run with: python3 -m max.voice_loop
"""
import os
import queue
import subprocess
...
from max import cli, control, hotkey, llm, sarvam_client, voice_events, wake_phrase
from max.config import load_config
from max.memory import MemoryStore
from max.tools import spotify
...
# Max looks completely dead before it says anything at all. That is the
# "Max is sleeping / sometimes it just doesn't respond" symptom, and it
...
# fired, the user paused to gather the sentence, and Max had already
...
# How long after Max stops speaking its own voice can still be arriving
...
# Spoken "speak in Assamese / English" is mapped to /assamese and
# /english in cli.apply_language_trigger (same function process_turn
# uses, so HUD-typed Assamese "স্পিক ইংলিছ" also switches).
...
# "Max, can you check my email") is never swallowed by this, only a
...
# the defaults for "hey max", including how Whisper actually renders it
...
configure_wake_phrases(["hey max"])
...
def _strip_leading_wake_phrase(text: str) -> str:
    ...
    _BARE_WAKE_PHRASES (accent-stripped, fused words, STT variants of
    "Max"), but as a prefix rather than a whole-utterance match. Seen
    ...
    literally starting with the mangled name plus "run the command, sleep 30" ran
...
def _ack_tone_path() -> str:
    ...
    Max had no way of telling the user it had started listening, so
    ...
    path = os.path.join(tempfile.gettempdir(), "max_ack_tone.wav")
...
def _looks_like_self_echo(new_text: str, last_reply: str) -> bool:
    """True if new_text is plausibly Max's own just-spoken reply bleeding
    back through the mic, not a new thing the user said.

    Seen live: no acoustic echo cancellation means the mic can pick up
    Max's own speaker output. That gets transcribed as if it were a
    ...
    it's Max answering a mangled transcript of itself).
    ...
    Seen live: Max's own longer replies (especially tool results like a
    ...
    # answers a question using the question's own words — Max asked
    ...
    if n < _ECHO_MIN_WORDS:
        return False
    # "yes, can you show me" after Max offered to show something is a
    # real follow-up. Fuzzy overlap used to throw it away because the
    # reply contained show/you/me — seen live, the pointer never moved.
    joined = " ".join(new_words)
    if re.search(r"\b(show|point|guide|cursor)\b", joined):
        return False
...
class VoiceLoop:
    def __init__(self, cfg: dict):
        ...
        # When Max last stopped talking — the echo filter's time window.
        ...
        # Live switches shared with the web UI (max/control.py).
        ...
        configure_wake_phrases(cfg.get("wake_phrases") or ["hey max"])
        ...
        if self.wake_mode == "phrase":
            phrases = cfg.get("wake_phrases") or ["hey max"]
            ...
            # a smaller one (see max/wake_phrase.py).
    ...
    def _on_wake_hotkey(self) -> None:
        ...
        saying the wake phrase — Max chimes and listens for one utterance
        ...
        if self._control.mic_muted:
            ...
            # Max can only be revived from the web UI, which is exactly
    ...
    def _on_push_to_talk(self) -> None:
        ...
        barge-in, so pressing the key while Max is talking stops it and
    ...
    def calibrate(self, seconds: float = 2.5) -> None:
        ...
        base = self._base_silence_rms_threshold
        # Calibrating while music or speech is already in the room used
        # to push the floor to 5000+ (seen live: 2389 median -> 5255).
        # After that, follow-up listens treated the song as speech and
        # Max answered lyrics instead of the user. If the "ambient"
        # window is already loud, keep the configured floor.
        if median > base * 3 or p90 > base * 5:
            print(
                f"  ambient RMS median {median:.0f} / p90 {p90:.0f} looks like "
                f"speech/music — keeping floor {base:.0f}"
            )
            self.silence_rms_threshold = base
        else:
            cap = base * 2.5
            calibrated = min(max(base, median * 2.2), cap)
            print(
                f"  ambient RMS median {median:.0f} / p90 {p90:.0f} "
                f"-> silence threshold set to {calibrated:.0f}"
            )
    ...
    def _watch_for_barge_in(
        ...
        Max's own speakers. An energy threshold cannot tell that apart
        from the user interrupting, and measured over 416 real turns it
        fired 179 times (43%) — Max cutting its own replies into
        ...
        impossible instead: Max does not say its own wake phrase, so its
        ...
        is "Max never says it" rather than "no model can fire on it" —
        ...
        while not stop_event.is_set():
            ...
            if getattr(self, "_ptt_interrupt", False):
                ...
                return
            if self.wake_mode == "phrase":
                # Only here: "stop Max" while it is talking or working has
    ...
    def _play_ack_tone(self) -> None:
        ...
        # same path that made Max's speech bleed into recordings.
        with self._barge_frames_lock:
    ...
    def record_utterance(
        ...
        # including the entire time Max was mid-reply (LLM latency +
        ...
        # of Max's OWN just-finished speech before ever reaching
        ...
        self._last_max_vad = max_vad_prob
        recorded = np.concatenate(frames) if frames else np.array([], dtype=np.int16)
    ...
    def _speak_streaming(
        ...
        silence before Max said a single word. Synthesis of chunk N+1
        ...
        try:
            ...
            # _barge_frames, including all the frames that are Max's own
            ...
            # audio_q but has never touched this. So the tail of Max's
            ...
            # captured after Max stopped talking, while still doing its
            # real job of catching a user who starts speaking early.
            with self._barge_frames_lock:
                self._barge_frames.clear()

    def transcribe(self, audio: np.ndarray, language_code: str | None = None) -> str:
        if audio.size == 0:
            return ""
        # Whisper small.en cannot hear Assamese (or Hindi). Once the
        # conversation is in a Sarvam language, send the utterance to
        # Saaras instead so the user can actually talk back.
        if language_code in ("as-IN", "hi-IN"):
            wav_bytes = sarvam_client.pcm16_to_wav(
                np.ascontiguousarray(audio, dtype=np.int16).tobytes(),
                SAMPLE_RATE,
            )
            try:
                text, detected = sarvam_client.speech_to_text(
                    wav_bytes, language_code=language_code, mode="codemix"
                )
            except sarvam_client.SarvamError as e:
                print(f"(Sarvam STT unavailable: {e})")
                return ""
            if detected and detected != language_code:
                print(f"  (Saaras heard {detected})")
            return text
        ...
        while True:
            ...
            try:
                with sd.InputStream(
                    ...
                    if first_start:
                        first_start = False
                        from max import screen_capture
                        if screen_capture.ensure_permission():
                            print("  screen: Screen Recording is on")
                        else:
                            print("  screen: " + screen_capture.PERMISSION_HELP)
                        print(f'Max voice loop online, {self.cfg["user_name"]}. Say "{wake_label}" to begin.\n')
                    else:
                        print(f'Max back online, {self.cfg["user_name"]}. Say "{wake_label}" to begin.\n')
                ...
                print("\n🔇 Microphone OFF — Max is not listening. "
    ...
    def _conversation_loop(self, mem: MemoryStore) -> None:
        ...
        while True:
            try:
                ...
                # log), leaving the user with a Max that looked "on" but
    ...
    def _keep_listening(self) -> None:
        """Stay in the conversation after a name-only wake.

        `_force_wake_listen` only changed the *next record timeout*. The
        next turn still called listen_for_wake_word() because the
        conversation window is set at the *end* of a completed turn —
        name-only returns early, so the window was never opened. Seen
        live: "Hey Oren" chimed, then the actual command was ignored
        until they said the wake phrase again.
        """
        self._force_wake_listen = True
        window = self.cfg.get("voice_conversation_window_seconds", 45)
    ...
    def _run_one_turn(self, mem: MemoryStore, last_reply_text: str) -> str:
        already_in_conversation = (
            getattr(self, "_force_wake_listen", False)
            or (
                self.conversation_active_until is not None
                and time.monotonic() < self.conversation_active_until
            )
        )
    ...
    def _handle_turn_audio(
        ...
        # when they called Max by name mid-conversation, which is the
        ...
        if pending is not None and pending.size:
            ...
            # "hey max, what's the weather" is already recorded in full by
        ...
        if not heard_speech:
            ...
            max_vad = getattr(self, "_last_max_vad", 0.0)
            if peak <= self.silence_rms_threshold:
                print("(Didn't catch anything.)\n")
                return last_reply_text
            # High RMS + low VAD is the room (Spotify, TV), not a missed
            # utterance. Transcribing it produced live lyric fragments
            # ("I'll keep on feeling high") that looked like Max had
            # gone mad — and blocked the real command behind them.
            if max_vad < 0.35:
                print(
                    f"(Ignoring — loud but not speech, VAD {max_vad:.2f} "
                    f"RMS {peak:.0f}.)\n"
                )
                return last_reply_text
            print(f"  (VAD said no speech but peak RMS {peak:.0f} / VAD {max_vad:.2f} "
                  f"is above the noise floor {self.silence_rms_threshold:.0f} — transcribing anyway)")
        text = self.transcribe(audio, language_code=cli.get_conversation_language(mem))
        ...
        if already_in_conversation and _is_bare_wake_phrase(text):
            ...
            # so calling Max by name during an open window produced
            # nothing observable whatsoever. Seen live, the user said "Hey
            # Max" four times in a row and got silence every time, which
        ...
        if not text.strip():
            # Nothing left once the name came off: they called Max and
        ...
        # an echo: Max's voice reaching the mic while, or just after, it
        ...
        # words. From the user's side Max had simply gone deaf. A
        ...
        # can never be enough.
        #
        #   1. If they woke Max by name for this turn, it is not an echo.
        #      Saying the wake phrase is proof of intent; Max's own voice
        #      never contains it.
        #   2. Otherwise it must arrive within ECHO_WINDOW_SECONDS of Max
        ...
        if INTERRUPT_RE.search(normalized) and not _NOT_AN_INTERRUPT_RE.search(normalized):
            ...
            # loop process — you'd have to restart Max just to
        ...
        # triggers on the wake word rather than on loudness, and Max's
        ...
        try:
            backend, response = cli.process_turn(
                cli.apply_language_trigger(text),
                self.cfg,
                mem,
                interactive=True,
        ...
        if self._stop_requested:
            # "Stop Max" means be quiet, not "I'm about to say something
...
_SINGLETON_PID = Path("/tmp/max_voice_loop.pid")


def _acquire_singleton() -> bool:
    """Refuse to start a second listener. Two loops on one mic is how
    the HUD 'Start Voice' button made Max randomly deaf."""
    if _SINGLETON_PID.exists():
        try:
            old = int(_SINGLETON_PID.read_text().strip())
        except ValueError:
            old = 0
        if old and old != os.getpid():
            try:
                ...
                print(
                    f"Another voice loop is already running (pid {old}). "
                    "Not starting a second one."
                )
                return False
    _SINGLETON_PID.write_text(str(os.getpid()))

    def _release() -> None:
        try:
            if _SINGLETON_PID.read_text().strip() == str(os.getpid()):
                _SINGLETON_PID.unlink(missing_ok=True)
        except OSError:
            pass
...
def main():
    if not _acquire_singleton():
        sys.exit(0)
    from max.security import harden_home
    harden_home()
    cfg = load_config()
    active_loop = None
    pending_wake = False
