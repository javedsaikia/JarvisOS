"""WebSocket bridge for the Max web UI.
...
or confirmation logic lives here — that's all in max/router.py and
max/cli.py, completely untouched by this file. If this file disappeared,
`python3 -m max.cli` and `python3 -m max.voice_loop` would be
...
Run with: max/.venv/bin/python3 -m max.bridge
"""
import asyncio
import base64
import json
...
from max import cli, control, llm, tts, voice_events
from max.config import load_config, update_config
from max.memory import LOG_PATH, MemoryStore
from max import screen_capture, security
from max.tools import spotify

# Loopback only — never 0.0.0.0. Caddy proxies 127.0.0.1:8765.
HOST = "127.0.0.1"
PORT = 8765
PROJECT_ROOT = Path(__file__).resolve().parent.parent
# Same path the standalone `python -m max.voice_loop` CLI process has
...
def _is_voice_loop_pid(pid: int) -> bool:
    """True only for `python -m max.voice_loop`, not a shell that
    happens to mention that string in its command line."""
    try:
        ...
        return False
    return "-m max.voice_loop" in cmd


def _voice_pids() -> list[int]:
    try:
        out = subprocess.check_output(["pgrep", "-f", "max.voice_loop"], text=True)
    ...
    for raw in out.split():
        try:
            ...
            continue
        if pid != me and _is_voice_loop_pid(pid):
            pids.append(pid)
    return pids


def _voice_running() -> bool:
    """True if *any* voice loop is alive — including one started from
    Terminal via ./start_max.sh, which this process did not spawn.

    Seen live: restarting the bridge dropped `_voice_proc`, the HUD
    showed Start Voice / Loop offline, the user clicked Start, and a
    second loop fought the first for the microphone. That is the
    'sometimes it responds, sometimes it doesn't' failure.
    """
    global _voice_proc
    if _voice_proc is not None and _voice_proc.poll() is None:
        return True
    _voice_proc = None
    return bool(_voice_pids())


def _signal_voice(sig: int) -> bool:
    sent = False
    global _voice_proc
    if _voice_proc is not None and _voice_proc.poll() is None:
        try:
            _voice_proc.send_signal(sig)
            sent = True
        except (ProcessLookupError, OSError):
            pass
    for pid in _voice_pids():
        try:
            os.kill(pid, sig)
            sent = True
        except OSError:
    ...
    with _voice_lock:
        ...
        try:
            _voice_proc = subprocess.Popen(
                [sys.executable, "-u", "-m", "max.voice_loop"],
    ...
    with _voice_lock:
        if _voice_proc is not None and _voice_proc.poll() is None:
            ...
            try:
                ...
                _voice_proc.wait(timeout=5)
        _voice_proc = None
        # Also reap a loop this process did not spawn (Terminal
        # ./start_max.sh). Leaving it up is how a later Start Voice
        # created a second listener.
        deadline = time.monotonic() + 5
        while time.monotonic() < deadline:
            leftover = _voice_pids()
            if not leftover:
                break
            for pid in leftover:
                try:
...
def _voice_interrupt() -> bool:
    ...
    with _voice_lock:
        if not _voice_running():
            return False
        return _signal_voice(signal.SIGUSR2)
...
async def _handle_message(ws, loop: asyncio.AbstractEventLoop, confirm: WSConfirm, text: str) -> None:
    ...
    if streamed:
        ...
        if not closed:
            ...
            try:
                await ws.send(json.dumps(_memory_payload()))
            except ConnectionClosed:
                pass
        return
...
def _memory_payload() -> dict:
    """Facts and a few profile lines for the HUD Memory card."""
    facts: list[str] = []
    for line in mem.load_facts().splitlines():
        stripped = line.strip()
    ...
    style: list[str] = []
    try:
        style = (mem.load_profile_sections().get("Speaking style") or [])[:4]
    except Exception:
        pass
    return {"type": "memory_state", "facts": facts[:12], "style": style}
...
async def _voice_events_poll_loop() -> None:
    ...
    played (see max/voice_events.py) so the orb moves with real speech
    ...
    what marks "Max started talking now".
    """
    loop = asyncio.get_running_loop()
    ...
    while True:
        ...
        for event in events:
            if event.get("type") == "speaking":
                ...
                # sent on the fallback capture path (max/screen_capture.py
...
def _capture_feed_frame() -> bytes:
    ...
    """
    with _cfg_lock:
        marker = cfg.get("screen_hud_window_marker", "Max HUD")
        ...
        for entry in entries:
            ...
            for client in list(_connected_clients):
                try:
                    ...
                    _connected_clients.discard(client)
        try:
            await _broadcast(json.dumps(_memory_payload()))
        except Exception:
            pass
        ...
        async for raw in ws:
            ...
            if mtype == "confirm_response":
                ...
                if _set_model_role(msg.get("role", ""), msg.get("model", "")):
                    ...
                    await ws.send(json.dumps({
                        "type": "error",
                        "message": "That model can't be selected — check the API key in max/.env.",
                    }))
...
async def main() -> None:
    ...
    print(f"Max bridge listening on ws://{HOST}:{PORT} (loopback only)")
    ...
    async with websockets.serve(
        ...
        HOST,
        PORT,
        process_request=security.websocket_process_request,
    ):
