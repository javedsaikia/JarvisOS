...
cli.handle_spotify's local now-playing read, since that reads this Mac's
...
from max import spotify_client
from max.applescript import run
...
# Last real search string the user asked for. "play it" / a spoken "yes"
# after a missed route reuse this instead of searching the pronoun.
_last_query: str | None = None
_pending_play: bool = False


def remember_query(query: str) -> None:
    global _last_query, _pending_play
    q = (query or "").strip()
    if not q:
        return
    _last_query = q
...
def has_pending_play() -> bool:
    return _pending_play and bool(_last_query)
...
def play_track(query: str) -> str:
    ...
    real track via the Spotify Web API (max/spotify_client.py — the
    ...
    remember_query(q)
    try:
        match = spotify_client.search_track(q)
    ...
    for _ in range(8):
        ...
        if _player_state() == "playing":
            actual = _now_playing_line()
            break
    clear_pending_play()
    # Report what the app is actually playing. Saying the search hit while
    # a leftover track kept going is how "Nothing Else Matters" was
    # announced as High Hopes — or the other way around.
    if actual:
        wanted = f"{match['name']} {match['artist']}".lower()
        if match["name"].lower() not in actual.lower() and match["artist"].lower() not in actual.lower():
            return (
                f'Spotify is playing {actual}, not {match["name"]} by {match["artist"]}. '
                "Say the title again and I'll retry."
            )
        return f"Playing {actual}."
...
def pause_for_conversation() -> None:
    ...
    music doesn't bleed into the recording or talk over Max's reply.
...
def _apply_volume_cap() -> None:
    """Caps the volume when Max itself starts or resumes playback — seen
    ...
    Max starts playback; explicit volume_up/volume_down/mute commands
