...
_USER_AGENT = "Max/1.0 (local personal assistant)"
...
# Each category is one or more OSM tags. Restaurants in smaller Indian
# towns are often tagged amenity=fast_food or food_court, not restaurant,
# so a single-tag search silently returned an empty list while the model
# then invented names.
CATEGORY_TAGS = {
    "restaurant": [
        ("amenity", "restaurant"),
...
def geocode_place(name: str) -> dict | None:
    """Named city → coordinates via Nominatim. Used only when the user
    explicitly asks for weather somewhere else — bare 'what's the weather'
    always uses GPS."""
    query = (name or "").strip()
    if not query:
        return None
...
def get_weather(lat: float, lon: float) -> dict:
    ...
    rain_today = _rain_line(data.get("daily") or {})
    return {
...
def _rain_line(daily: dict) -> str:
    """One spoken sentence about today's rain, or empty if unknown."""
    ...
    if prob is None and amount is None:
        return ""
    if (prob is not None and prob >= 40) or (amount is not None and amount > 0.2):
        if prob is not None:
            return f"Rain is likely today ({prob:.0f}% chance)."
        return "Some rain is expected today."
...
def _category_tags(category: str) -> list[tuple[str, str]]:
    ...
    # Older callers stored a single (key, value) pair.
