"""What Max knows about you, kept locally and injected into every model.
...
Groq or Gemini changes who is thinking, never who Max thinks you are.

Three files, with deliberately different owners:

- `facts.md` — **yours**. Written by hand and by `/remember`. Nothing
  automatic ever edits it. If Max's self-written notes go wrong, this is
  the copy that is still exactly what you said.
- `profile.md` — **Max's**. Distilled automatically from conversations
  (see max/reflection.py): how you speak, what you prefer, what you
...
PROFILE_HEADER = """# Max's profile of you

Written automatically from conversations (max/reflection.py). Safe to
...
class MemoryStore:
    def __init__(self):
        ...
        if not FACTS_PATH.exists():
            FACTS_PATH.write_text("# Max Memory\n\n## Facts & Preferences\n")
        from max.security import harden_dir, harden_file
        harden_dir(MEMORY_DIR)
        for path in (FACTS_PATH, PROFILE_PATH, LOG_PATH, ARCHIVE_PATH, STATE_PATH):
            harden_file(path)
    ...
    def load_profile_sections(self) -> dict[str, list[str]]:
        ...
        supported way to correct what Max believes.
        """
        ...
        for line in self.load_profile().splitlines():
            ...
            if stripped.startswith("## "):
                ...
                # into prompts as something Max "knows".
    ...
    def context_block(self) -> str:
        ...
        "what Max knows about you" cannot differ between models — that is
