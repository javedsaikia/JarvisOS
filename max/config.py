...
DEFAULTS = {
    ...
    "voice_ollama_max_tokens": 80,
    "bridge_ollama_model": "qwen2.5-coder:1.5b",
    "bridge_ollama_max_tokens": 120,
    # Hard cap on ordinary chat (HUD + voice fallback). Stops Groq writing
    # a lecture when the user asked a one-line question.
    "chat_max_tokens": 120,
    "ollama_keep_alive": "30m",
    # Models the dashboard offers, and which one answers for each job.
    # Cloud entries need their key in max/.env (GROQ_API_KEY,
    ...
    "model_roles": {"chat": "local", "voice": "local-fast", "screen": "local-fast",
                    ...
                    # permanent context.
                    "memory": "local"},
    # --- Memory (max/memory.py + max/reflection.py)
    ...
    # Automatic profile-building. Off means Max only knows what you told
    ...
    # enough to miss real wakes. See max/wake_phrase.py.
    "wake_stt_model": "",
    ...
    # Push-to-talk (see max/hotkey.py). Hold the combo and speak — no
    ...
    # Tap a shortcut to wake Max, instead of holding one to talk (see
    # max/hotkey.py). Works from any app, so you never have to find the
    ...
    # up as a Max follow-up command for minutes after the last real
    # exchange, with no wake word required to re-arm it.
    "voice_conversation_window_seconds": 45,
    ...
    # --- "What's on my screen?" (max/screen_capture.py + tools/vision.py)
    ...
    # Window title marked as Max's own and left out of the capture, so
    # the HUD is never what Max describes.
    "screen_hud_window_marker": "Max HUD",
    ...
    # Isolated Playwright Chromium is not the user's real desktop browser
    # and is not daily computer control. Off unless you turn it on.
    "browser_enabled": False,
    "location_enabled": True,
    "location_default_radius_m": 3000,
    "calling_enabled": True,
    # Javed's iPhone — Continuity uses this handset as the cellular radio.
    # Outgoing restaurant calls are never placed as this number.
    "user_phone": "",
    "news_enabled": True,
    "spotify_max_volume": 75,
    # Stable Composio user id — connections are stored against this.
    # Overwritten at runtime with the user id that already owns
    # Composio connections, if one exists.
    "composio_user_id": "max",
}
